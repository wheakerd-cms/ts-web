import type {AppConfigStateInterface} from "@/config/types";
...
const getAppName = (): string => {
    ...
    const appName: string | undefined = state.mode === 'host' ? matchReg?. [2] : matchReg ?. [1];

    return !!appName && Object.keys(appConfig.app).includes(appName)
        ? appName : appConfig.defaultApp;
};

const getLayoutView = (): string | undefined => {
